...
class Brands extends CI_Controller {
...

	{
		...
		$data['recordset'] 		= $this->ModelBrands->getList($config,$start,$param);
	...
	function addedit($id = 0)
	{
		...
		if($contentId > 0)
		{
			...
			$data['action'] = "Edit";
			//=================prepare DATA ==================
			$rs = $this->ModelBrands->getSingle($contentId,'brands');
			//$row = $rs->fields;
			if(is_array($rs[0]))
			{
	...
	function do_addedit()
	{
		...
		if($_FILES['icon']['size'] > 0){
			...
			$config['upload_path'] 	 = file_upload_absolute_path().'brands/';
		...
		if($contentId > 0)   //edit
		{
			$affected_row = $this->ModelBrands->editContent($contentId,$data,'brands');
		...
		{	
			$insert_id = $this->ModelBrands->addContent($data,'brands');
			if($insert_id)
			{
	...
	function delete()
	{
		...
		$file = $this->functions->getNameTable('brands','icon','id',$id,'','');	
		$this->ModelBrands->delete($id,'brands');
		$path= file_upload_absolute_path().'brands/'.$file;
		unlink($path);
		$this->nsession->set_userdata('succmsg', 'Brands Deleted.');
	...
	function inactive()
	{
		...
		$this->functions->checkAdmin($this->controller.'/');
		$id = $this->uri->segment(3, 0);		
		$this->ModelBrands->inactive($id,'category_level_1');		
	...
	function activate()
	{
		...
		$this->functions->checkAdmin($this->controller.'/');
		$id = $this->uri->segment(3, 0);		
		$this->ModelBrands->activate($id,'category_level_1');		
		$this->nsession->set_userdata('succmsg', 'Successfully Template Inactivated.');
		redirect(base_url($this->controller."/index/"));
		return true;
		}
	...
	function changeorder(){
		...
		if($this->input->post('val') || $this->input->post('id')){
			...
			$result=$this->ModelBrands->changeorder($val,$id,'category_level_1');
