...
class Utilities extends CI_Controller {
	...
	function index()
	{
		...
		$data['recordset'] 		= $this->Modelutilities->getList($config,$start,$param);
		...
		$data['module'] = 'utilities Management';
	...
	function addedit($id = 0)
	{
		...
		if($contentId > 0)
		{
			...
			$data['action'] = "Edit";
			//=================prepare DATA ==================
			$rs = $this->Modelutilities->getSingle($contentId);
			//$row = $rs->fields;
			if(is_array($rs[0]))
			{
		...
		$elements['menu'] = 'menu/index';
		$elements['topmenu'] = 'menu/topmenu';
		$elements['main'] = 'utilities/add_edit';
		$element_data['main'] = $data;
		$element_data['main'] = $data;
	...
	function do_addedit()
	{
		...
		if($contentId > 0)   //edit
		{
			$affected_row = $this->Modelutilities->editContent($contentId,$data);
		...
		{	
			$insert_id = $this->Modelutilities->addContent($data);
			if($insert_id)
			{
	...
	function delete()
	{
		...
		$this->Modelutilities->delete($id);
		
		$this->nsession->set_userdata('succmsg', 'Successfully utilities Deleted.');
	...
	function inactive()
	{
		$this->functions->checkAdmin($this->controller.'/');
		$id = $this->uri->segment(3, 0);		
		$this->Modelutilities->inactive($id);		
	...
	function activate()
	{
		$this->functions->checkAdmin($this->controller.'/');
		$id = $this->uri->segment(3, 0);		
		$this->Modelutilities->activate($id);		
		$this->nsession->set_userdata('succmsg', 'Successfully Template Inactivated.');
		redirect(base_url($this->controller."/index/"));
		return true;
	}
	...
	function changeorder(){
		...
		if($this->input->post('val') || $this->input->post('id')){
			...
			$result=$this->Modelutilities->changeorder($val,$id);
