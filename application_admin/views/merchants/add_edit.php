...
var company_country='<?php echo isset($company_country)?$company_country:''; ?>';
if(company_country !=0){
  getState(company_country,'company_state');
}


  function getState(val,idt){
  	if(idt=='company_state'){
  		var state='<?php echo isset($company_state)?$company_state:0; ?>';
  	}else{
   ...
   // alert(state);
    if(val!=''){
      var url='<?php echo base_url(); ?>merchants/getState/'+val+'/'+state;
      $.ajax({
        type: 'POST',
        url: url,
  ...
  function getCity(val,idt){
    if(idt=='company_city'){
  		var city='<?php echo isset($company_city)?$company_city:0; ?>';
  	}else{
  		var city='<?php echo isset($city)?$city:0; ?>';
  	}
    if(val!=''){
      var url='<?php echo base_url(); ?>merchants/getCity/'+val+'/'+city;
      $.ajax({
        type: 'POST',
        url: url,
        success: function(result){
	...
	$("form[name='form1']").validate({
      ...
      rules: {
        ...
        username:{
          required: true,
          remote: {
	            url: "<?php echo base_url(); ?>merchants/checkuser/username/<?php echo $id; ?>",
	            type: "post"
	      }
        },
        email:{
          ...
          email: true,
	      remote: {
	            url: "<?php echo base_url(); ?>merchants/checkuser/email/<?php echo $id; ?>",
	            type: "post"
	      }
        },
        phone:{
          ...
          number: true,
          remote: {
	            url: "<?php echo base_url(); ?>merchants/checkuser/phone/<?php echo $id; ?>",
	            type: "post"
	      }
        },
        <?php if($action=='Add'){ ?>
