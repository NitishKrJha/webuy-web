...
class ModelHome extends CI_Model {
    ...
    function getList(&$config,&$start,&$param)
    {
        ...
        $this->db->select('adsection.*');

        $recordSet = $this->db->get('adsection');
        $config['total_rows'] = 0;
    ...
    function addContent($data)
    {
        ...
        $max_num=$this->db->select('MAX(sort_num) as max_num')->get_where('adsection')->row()->max_num;
        ...
        $this->db->insert('adsection',$insert_data);
    ...
    function editContent($id,$data)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('adsection', $update_data);
    ...
    function delete($id)
    {
        $this->db->delete('adsection', array('id' => $id));
        return true;
    }

    function getSingle($id)
    {
        $sql = "SELECT * FROM adsection WHERE id = ".$id;
    ...
    {
        $sql = "UPDATE adsection SET is_active = '1' WHERE id = ".$id."";
        $recordSet = $this->db->query($sql);
    ...
    {
        $sql = "UPDATE adsection SET is_active = '0' WHERE id = ".$id."";
        $recordSet = $this->db->query($sql);
    ...
    function changeorder($val,$id){
        ...
        $getFaq=$this->db->select('id,sort_num')->get_where('adsection',array('id'=>$id))->row_array();
        if(count($getFaq) > 0){
            $prev_val=$getFaq['sort_num'];
            $result['intrchng_val']=$prev_val;
            $getFaqPre=$this->db->select('id,sort_num')->get_where('adsection',array('sort_num'=>$val))->row_array();
            if(count($getFaqPre) > 0){
                $intrchng_id=$getFaqPre['id'];
                $result['intrchng_id']=$intrchng_id;
                $this->db->update('adsection',array('sort_num'=>$prev_val),array('id'=>$intrchng_id));

            }


            $this->db->update('adsection',array('sort_num'=>$val),array('id'=>$id));
