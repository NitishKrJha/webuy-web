...
class ModelSubscriber extends CI_Model {
	...
	function getList(&$config,&$start,&$param)
	{
		...
		//==============================================================
		$this->db->select('COUNT(subscriber.id) as TotalrecordCount');
	...
	function changeorder($val,$id,$tbl_name){
		...
		if(count($getFaq) > 0){
			...
			if(count($getFaqPre) > 0){
				...
				$this->db->update('subscriber',array('sort_num'=>$prev_val),array('id'=>$intrchng_id));
